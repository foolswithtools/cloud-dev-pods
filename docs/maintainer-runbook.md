...
### Release App setup (one-time)

`release.yml` authenticates as a dedicated GitHub App rather than the runner's `GITHUB_TOKEN`. This is necessary because PRs opened by `GITHUB_TOKEN` do not trigger workflow runs (GitHub anti-recursion safeguard) — the release PR's required status checks would never report and the PR would be unmergeable. App-token-authored PRs trigger workflows normally.

**Setup** (org-owner action, one time):

1. <https://github.com/organizations/foolswithtools/settings/apps> → "New GitHub App".
2. Name: `cloud-dev-pods-release-bot`. Homepage URL: anything (e.g., the repo URL). Webhook: **disabled** (uncheck "Active").
3. Repository permissions:
   - **Contents**: Read and write (push the release branch and tag)
   - **Pull requests**: Read and write (open the release PR)
   - **Metadata**: Read (default, can't be unset)

   Leave everything else at "No access".
4. "Where can this GitHub App be installed?": **Only on this account**.
5. Create the App. From the App's settings page, note the **App ID**, then "Generate a private key" — downloads a `.pem` file. Treat it like an SSH key: never commit it.
6. Install the App on the repo: App page → "Install App" → pick the `foolswithtools` org → "Only select repositories" → check `cloud-dev-pods` → Install.
7. Add the credentials to the repo:

   ```bash
   gh variable set RELEASE_APP_ID --repo foolswithtools/cloud-dev-pods --body "<app id from step 5>"
   gh secret   set RELEASE_APP_PRIVATE_KEY --repo foolswithtools/cloud-dev-pods < /path/to/downloaded.pem
   ```

8. Delete the local `.pem` once the secret is set (`shred -u` or `srm` on macOS, or just `rm` if you trust your filesystem).

**Verification**: push any conventional commit (`feat:` or `fix:`) to `main`. The `release` workflow should succeed and the release-please PR should show CI checks running (not stuck on "Expected — Waiting for status to be reported").

**Failure modes**:

- `Error: Input required and not supplied: app-id` → `RELEASE_APP_ID` Variable not set on the repo.
- `Error: failed to get installation token: ... 401 Unauthorized` → wrong/rotated private key, or the App was never installed on `cloud-dev-pods`.
- Release PR opens but no CI checks fire → `release.yml` is still referencing `secrets.GITHUB_TOKEN` somewhere; the App token must be passed as `${{ steps.app-token.outputs.token }}`.

**Key rotation**: GitHub App private keys don't auto-expire, but rotating annually is best practice. Generate a new key from the App's settings page, update `RELEASE_APP_PRIVATE_KEY`, then delete the old key from the App. No workflow change needed.
...
- **`secrets.GITHUB_TOKEN` anti-loop.** GitHub deliberately does not trigger workflow runs from PRs opened or pushed by the default `GITHUB_TOKEN`. Symptom: a bot-authored PR sits with no checks reported (status shows "Expected — Waiting for status to be reported"). `release.yml` works around this by authenticating as a dedicated GitHub App (see "Release App setup"); App-token-authored PRs do trigger workflows. Other bots (e.g., dependabot in some configurations) can still hit this — workaround is to close + immediately reopen the PR from a human identity, which re-fires `pull_request` with a non-bot actor.
